...
if (empty($dob)) {
    ...
    if ($ageInMonths < 48 || $ageInMonths > 54) {
        $errors['dob'] = "Age must be between 4 and 4.5 years.";
    }
}
...
if (empty($selectedSubjects)) {
    ...
    if (count($errors) == 0) 
    {
        ...
        foreach ($errors as $field => $message) {
            $errorMessages[] = array("field" => $field, "message" => $message);
        }
        
        echo json_encode(array("status" => "error", "errors" => $errorMessages));
    }
