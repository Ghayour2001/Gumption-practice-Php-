...
for ($i = 0; $i < count($cities); $i++) {
    ...
    $sql_city = mysqli_query($connection, "UPDATE studentcity SET city = '$city', province = '$province' WHERE fkstudentid = '$studentId'");
    
    if ($sql_city === false) {
        $_SESSION['toastr'] = array(
            'type' => 'error',
            'message' => 'Error updating studentcity data: ' . mysqli_error($connection),
            'title' => 'Error!'
        );
    }
}
