            ...
            <div class="col-md-10 offset-1">
                ...
                <form id="student-form" method="post" action="insert.php" enctype="multipart/form-data">
                    ...
                    <div class="main2 col-md-12 mt-3 mb-5 border border-info">
                        ...
                        <button type="button" id="addButton" class="btn btn-primary addButton"> Add </button>
                        ...
                        <table id="dataTable" class="table">
                            ...
                            <tbody id="subjecttbl">
                                ...
                                <?php
                        if (isset($row['subjectNames']) && isset($row['marks'])) 
                        {
                            $subjectNames = explode(',', $row['subjectNames']);
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td><?php echo $subjectName; ?><input type="hidden" name="subjectName[]" value="<?php echo $subjectName; ?>"></td>
                                    <td><?php echo $marks; ?><input type="hidden" name="marks[]" value="<?php echo $marks; ?>"></td>
...
$("#addButton").click(function() {
    ...
    if (subjectName && marks !== '') {
        var newRow = $("<tr>");
        var cols = "";
        cols += '<td>' + i + '<input type="hidden" name="sno[]" value="' + i + '"></td>';
        ...
        cols += '<td><button type="button" class="btn btn-warning edit">Edit</button></td>';
        cols += '<td><button type="button" class="btn btn-danger delete">Delete</button></td>';
        newRow.append(cols);
...
$("table").on("click", ".edit", function() {
    ...
    var subjectName = currentRow.find("td:eq(1)").text();
    var marks = currentRow.find("td:eq(2)").text();
    $("#subjectName").val(subjectName);
    $("#marks").val(marks);
    
    $("#addButton").text('Update').addClass('updateMode');
});


      
            $(document).on("click", ".updateMode", function() {
            //    alert(currentRow);
               alert('Updating...');
                $("#addButton").text('Add Subject').removeClass('updateMode');
                $("#subjectName").val('');
                $("#marks").val('');
            });
